...
void draw() {
    ...
    // 比例缩放方法(全局)
    SDL_Rect destRect_c = {0, 100, w, 100}; // x, y, w, h
    // 比例缩放
    static float x = 0;
    x += 0.1f;
    float scale = SDL_sinf(x);
    SDL_RenderSetScale(renderer, scale, scale); // x, y
    SDL_RenderCopy(renderer, image_Texture, NULL, &destRect_c);
